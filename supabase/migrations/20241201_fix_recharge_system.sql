-- Migration để sửa hệ thống nạp tiền
...
-- Tạo bảng mã nạp tiền (nếu chưa có)
...
-- Tạo bảng số dư người dùng (nếu chưa có)
...
-- Tạo bảng lịch sử giao dịch (nếu chưa có)
...
-- Xóa các policy cũ nếu có để tránh conflict
DROP POLICY IF EXISTS "Admin can view all recharge codes" ON public.recharge_codes;
DROP POLICY IF EXISTS "Admin can create recharge codes" ON public.recharge_codes;
DROP POLICY IF EXISTS "Users can view their used codes" ON public.recharge_codes;
DROP POLICY IF EXISTS "Users can view own balance" ON public.user_balances;
DROP POLICY IF EXISTS "Admin can view all balances" ON public.user_balances;
DROP POLICY IF EXISTS "Users can view own transactions" ON public.transaction_history;
DROP POLICY IF EXISTS "Admin can view all transactions" ON public.transaction_history;
...
-- Tạo một số mã nạp tiền test
INSERT INTO public.recharge_codes (code, amount_vnd, created_by) 
VALUES 
  ('TEST1234', 50000, (SELECT id FROM public.profiles LIMIT 1)),
  ('DEMO5678', 100000, (SELECT id FROM public.profiles LIMIT 1))
